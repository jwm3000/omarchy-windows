...
The **All workspaces / Alle Arbeitsflächen** switch controls the scope of the main tiled ↔ floating button. It is enabled by default, preserving the original global behavior. Turn it off to control every workspace independently: any number of workspaces can remain in Floating Mode while the others stay tiled. When it is enabled again, the current workspace's tiled/floating state plus its focus-border and window-transparency preferences are applied to every workspace. Newly opened windows float only on enabled workspaces. The scope choice persists across restarts.
...
The **Titlebar transparency / Titelleisten-Transparenz** switch controls only the native titlebar. It is slightly transparent by default; turn the switch off for a fully opaque titlebar. This setting is global: changing it on one workspace changes the titlebars on every workspace. The preference applies immediately and persists across restarts.
